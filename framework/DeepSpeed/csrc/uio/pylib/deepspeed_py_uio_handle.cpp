...
static void _start_uio_thread(std::shared_ptr<struct deepspeed_uio_thread_t> ctxt) { ctxt->run(); }

deepspeed_uio_handle_t::deepspeed_uio_handle_t(int entries = 64, 
                                               uint32_t flags = 0, 
                                               uint32_t wq_fd = 0, 
                                               int num_threads)
    : //_uio_ctxt(new uio_context(int entries = 64, uint32_t flags = 0, uint32_t wq_fd = 0, )),
      _single_submit(single_submit),
      //_overlap_events(overlap_events),
      _num_threads(num_threads),
      _uio_config(int entries = 64, uint32_t flags = 0, uint32_t wq_fd = 0),
      _num_pending_ops(0),
      _pinned_tensor_mgr(new deepspeed_pin_tensor_t())
{
    for (auto i = 0; i < num_threads; ++i) {
        _thread_contexts.push_back(std::make_shared<deepspeed_uio_thread_t>(i, _uio_config));
    }

    for (auto& ctxt : _thread_contexts) {
        _threads.push_back(std::thread(_start_uio_thread, ctxt));
    }
}

deepspeed_uio_handle_t::~deepspeed_uio_handle_t()
{
    _stop_threads();
    for (auto& thr : _threads) { thr.join(); }
}

const int deepspeed_uio_handle_t::get_block_size() const
{
    return _uio_ctxt ? _uio_ctxt->_block_size : -1;
}

const int deepspeed_uio_handle_t::get_queue_depth() const
{
    return _uio_ctxt ? _uio_ctxt->_queue_depth : -1;
}

const bool deepspeed_uio_handle_t::get_single_submit() const { return _single_submit; }

const bool deepspeed_uio_handle_t::get_overlap_events() const { return _overlap_events; }

const int deepspeed_uio_handle_t::get_thread_count() const { return _num_threads; }

int deepspeed_uio_handle_t::read(torch::Tensor& buffer, const char* filename, const bool validate)
{
    ...
    if (_uio_config._overlap_events) {
        do_uio_operation_overlap(true, _uio_ctxt, xfer_ctxt, &_uio_config, nullptr);
    } else {
        do_uio_operation_sequential(true, _uio_ctxt, xfer_ctxt, &_uio_config, nullptr);
    }
    ...
    if (validate) { validate_uio_operation(true, filename, read_buffer, num_file_bytes); }
...
int deepspeed_uio_handle_t::write(const torch::Tensor& buffer,
                                  const char* filename,
                                  const bool validate)
{
    assert(_uio_ctxt);
    ...
    if (_uio_config._overlap_events) {
        do_uio_operation_overlap(false, _uio_ctxt, xfer_ctxt, &_uio_config, nullptr);
    } else {
        do_uio_operation_sequential(false, _uio_ctxt, xfer_ctxt, &_uio_config, nullptr);
    }
    const std::chrono::duration<double> uio_time =
        std::chrono::high_resolution_clock::now() - start_time;
...
void deepspeed_uio_handle_t::_schedule_uio_work(std::shared_ptr<struct io_op_desc_t> scheduled_op)
{
    for (auto& ctxt : _thread_contexts) {
        {
...
std::shared_ptr<struct io_op_desc_t> deepspeed_uio_handle_t::_wait_for_uio_work()
{
...
void deepspeed_uio_handle_t::_stop_threads()
{
...
int deepspeed_uio_handle_t::wait()
{
    ...
    while (_num_pending_ops > 0) {
        ...
        if (completed_op->_validate) {
            validate_uio_operation(completed_op->_read_op,
...
bool deepspeed_uio_handle_t::_is_valid_parallel_uio_op(const bool read_op,
...
int deepspeed_uio_handle_t::pread(const torch::Tensor& buffer,
                                  const char* filename,
                                  const bool validate,
                                  const bool async)
{
    ...
    if (!_is_valid_parallel_uio_op(true, num_file_bytes)) { return -1; }
    ...
    _schedule_uio_work(scheduled_op);
...
int deepspeed_uio_handle_t::pwrite(const torch::Tensor& buffer,
                                   const char* filename,
                                   const bool validate,
                                   const bool async)
{
    ...
    if (!_is_valid_parallel_uio_op(false, num_write_bytes)) { return -1; }
...
int deepspeed_uio_handle_t::sync_pread(torch::Tensor& buffer, const char* filename)
{
    return pread(buffer, filename, false, false);
}

int deepspeed_uio_handle_t::sync_pwrite(const torch::Tensor& buffer, const char* filename)
{
    return pwrite(buffer, filename, false, false);
}

int deepspeed_uio_handle_t::async_pread(torch::Tensor& buffer, const char* filename)
{
    return pread(buffer, filename, false, true);
}

int deepspeed_uio_handle_t::async_pwrite(const torch::Tensor& buffer, const char* filename)
{
    return pwrite(buffer, filename, false, true);
}

at::Tensor deepspeed_uio_handle_t::new_cpu_locked_tensor(const size_t num_elem,
                                                         const torch::Tensor& example_tensor)
{
    return _pinned_tensor_mgr->alloc(num_elem, example_tensor.scalar_type());
}

bool deepspeed_uio_handle_t::free_cpu_locked_tensor(torch::Tensor& locked_tensor)
{
