...
MLIR_DEFINE_EXPLICIT_TYPE_ID(::mlir::triton::cpu::Dialect)
namespace mlir {
namespace triton {
...
TritonCPUDialect::TritonCPUDialect(::mlir::MLIRContext *context)
    : ::mlir::Dialect(getDialectNamespace(), context, ::mlir::TypeID::get<TritonCPUDialect>()) {
  getContext()->loadDialect<triton::TritonDialect>();
  getContext()->loadDialect<amx::AMXDialect>();
  getContext()->loadDialect<x86vector::X86VectorDialect>();
  getContext()->loadDialect<tensor::TensorDialect>();
  initialize();
}

TritonCPUDialect::~TritonCPUDialect() = default;
...
SmallVector<unsigned> getThreadsPerWarp(Attribute layout) {
  ...
  if (auto mmaLayout = layout.dyn_cast<MmaEncodingAttr>()) {
    //todo: fix this, value for different models or another value
    if (mmaLayout.isX86Vector())
        return {4, 8};
    /*With AMX, the closest equivalent to a warp would be a tile of data that's 
    loaded into the tile registers and then used in matrix operations. 
    The size of these tiles can be 8x8, 16x16, or 32x8, depending on the specific 
...
bool isSharedEncoding(Value value) {
  ...
  if (auto tensorType = type.dyn_cast<RankedTensorType>()) {
    ...
    return encoding && encoding.isa<triton::cpu::SharedEncodingAttr>();
  }
  return false;
}
...
bool isExpensiveCat(CatOp cat, Attribute targetEncoding) {
  ...
  auto totalElemsPerThread = cpu::getTotalElemsPerThread(tensorTy);
  auto shape = tensorTy.getShape();
  auto elemTy = tensorTy.getElementType();
  auto newTotalElemsPerThread =
      cpu::getTotalElemsPerThread(targetEncoding, shape, elemTy);
...
//todo: fixme, get version of cpu
bool MmaEncodingAttr::isX86Vector() const { return getVersionMajor() == 4; }

bool MmaEncodingAttr::isAMX() const { return getVersionMajor() == 5; }
...
struct TritonCPUInferLayoutInterface
  ...
  verifyDotOpEncodingCompatibility(Operation *op, Attribute operandEncodingA,
    ...
    auto aEncoding =
        operandEncodingA.dyn_cast<triton::cpu::DotOperandEncodingAttr>();
    auto bEncoding =
        operandEncodingB.dyn_cast<triton::cpu::DotOperandEncodingAttr>();
...
LogicalResult ConvertLayoutOp::canonicalize(ConvertLayoutOp op,
  ...
  if (dstType.getEncoding().isa<triton::cpu::DotOperandEncodingAttr>() &&
      srcType.getEncoding().isa<triton::cpu::MmaEncodingAttr>())
    return mlir::failure();
  // for hopper MMAv3
  if (!op.use_empty()) {
    ...
    if (hasDotUser) {
      if (dstType.getEncoding().isa<triton::cpu::SharedEncodingAttr>() &&
          srcType.getEncoding().isa<triton::cpu::MmaEncodingAttr>())
        return mlir::failure();
    }
  }
  ...
  auto alloc_tensor = dyn_cast<triton::cpu::AllocTensorOp>(arg);
  if (alloc_tensor) {
    if (!triton::cpu::isSharedEncoding(op->getResult(0))) {
      return mlir::failure();
    }
    rewriter.replaceOpWithNewOp<triton::cpu::AllocTensorOp>(
        op, op->getResult(0).getType());
    return mlir::success();
  }
  // cvt(insert_slice(x), type2) -> insert_slice(cvt(x, type2))
  auto insert_slice = dyn_cast<triton::cpu::InsertSliceAsyncOp>(arg);
  if (insert_slice) {
    if (!triton::cpu::isSharedEncoding(op->getResult(0))) {
      return mlir::failure();
    }
    ...
    auto newArg = rewriter.create<triton::cpu::ConvertLayoutOp>(
        op->getLoc(), newType, insert_slice.getDst());
    rewriter.replaceOpWithNewOp<triton::cpu::InsertSliceAsyncOp>(
  ...
  auto extract_slice = dyn_cast<triton::cpu::ExtractSliceOp>(arg);
  if (extract_slice) {
    if (!triton::cpu::isSharedEncoding(op->getResult(0))) {
      return mlir::failure();
    }
    ...
    rewriter.replaceOpWithNewOp<triton::cpu::ExtractSliceOp>(
  ...
  if (llvm::isa<triton::cpu::ConvertLayoutOp>(arg)) {
    if (arg->getOperand(0).getDefiningOp() &&
        !triton::cpu::isSharedEncoding(arg->getOperand(0)) &&
        triton::cpu::isSharedEncoding(op.getOperand()) &&
        !triton::cpu::isSharedEncoding(op.getResult())) {
      return mlir::failure();
    }
    if (triton::cpu::isSharedEncoding(op.getOperand()) &&
        triton::cpu::isSharedEncoding(op.getResult())) {
      return mlir::failure();
    }
    auto srcType = op.getOperand().getType().cast<RankedTensorType>();
    auto srcShared =
        srcType.getEncoding().dyn_cast<triton::cpu::SharedEncodingAttr>();
    if (srcShared && srcShared.getVec() > 1)
      return mlir::failure();
    rewriter.replaceOpWithNewOp<triton::cpu::ConvertLayoutOp>(
